...
class DlgPropertiesDisk(wx.Dialog):
	''' Dialog o programie '''
	...
	def __init__(self, parent, item):
		...
		appconfig = AppConfig()
		size = appconfig.get('properties_disk_wnd', 'size', (300, 300))
		self.SetSize(size)

		position = appconfig.get('properties_disk_wnd', 'position')
		if position is None:
			self.Centre(wx.BOTH)
	...
	def _create_layout_page_main(self, parent):
		...
		bsizer.Add(_creata_label(panel, _("Name") + ":"), 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL)
	...
	def _on_close(self, evt=None):
		appconfig = AppConfig()
		appconfig.set('properties_disk_wnd', 'size',		self.GetSizeTuple())
		appconfig.set('properties_disk_wnd', 'position',	self.GetPositionTuple())
