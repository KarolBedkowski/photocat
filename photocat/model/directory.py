...
class Directory(CatalogFile, TreeItem):
	...
	def __folder_files_list(self, path):
		files_path = ((fname, os.path.join(path, fname))
				for fname in os.listdir(path)
				if not fname.startswith('.')
					and os.path.splitext(fname)[1].lower() \
						in FileImage.IMAGE_FILES_EXTENSION)
		files = sorted((fname, fpath)
	...
	def _load_caption_txt_process_file(self, file_name, file_data):
		...
		if update_obj.desc is None or len(update_obj.desc) == 0:
			desc = (file_data[key]
					for key in ('Title', 'Subtitle', 'Date', 'Desc')
					if key in file_data)
